...
function ChatPageInner() {
	...
	// Fetch vendors list when vendor profile is loaded to avoid authentication race conditions
	useEffect(() => {
		if (vendor) {
	...
	const getMemberDetails = (memberId: string | number) => {
		if (vendor && memberId.toString() === vendor.id.toString()) {
			return {
				...
				shopName: vendor.shop_name || "Owner",
				profileImg: vendor.profile_img,
				initials:
					vendor.name
						.split(" ")
						.map((n: string) => n[0])
		...
		const found = vendors.find(
			(v) => v.id.toString() === memberId.toString(),
		);
		if (found) {
			return {
				id: found.id,
				name: found.name,
				shopName: found.shop_name || "Independent Vendor",
				profileImg: found.profile_img,
				initials:
					found.name
						.split(" ")
						.map((n: string) => n[0])
		...
		return {
			id: memberId,
			name: `Vendor #${memberId}`,
			shopName: "Trade Partner",
			profileImg: null,
			initials: "VD",
		};
	};
	...
	useEffect(() => {
		if (activeChatId) {
			...
			fetchMessages(activeChatId, 15, 0).then((result) => {
				if (result) {
	...
	useEffect(() => {
		if (activeChatMessages.length > 0) {
			...
			// Scroll to bottom if this is the first load of a chat, or if a new message is appended to the bottom
	...
	return (
		<div className="h-full w-full flex rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md overflow-hidden shadow-xl animate-fadeIn">
				...
				<div className="p-4 border-b border-zinc-150 dark:border-zinc-800 flex flex-col gap-3">
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200/50 dark:border-zinc-700/50">
							<span
								className={`h-1.5 w-1.5 rounded-full ${isConnected ? "bg-emerald-505 bg-emerald-500 animate-pulse" : "bg-red-500"}`}
							/>
							...
							return (
								...
								>
									...
									<div className="flex-1 min-w-0">
										...
										<p
											className={`text-[11px] truncate pr-4 font-medium ${isCustomerTyping ? "text-primary dark:text-secondary italic" : "text-zinc-500 dark:text-zinc-400"}`}
										>
											{isCustomerTyping
												? "Typing..."
												: chat.lastMessage}
			...
			{activeChat ? (
				<div
					className={`flex-1 flex flex-col min-w-0 bg-zinc-50/20 dark:bg-zinc-950/10 ${
						!showMobileChat ? "hidden md:flex" : "flex"
					}`}
				>
					{/* Chat header */}
							...
							return (
								...
								>
									...
									>
										...
										{m.attachment && (
											<div
												className={`${m.text ? "mt-3 pt-3 border-t border-zinc-200/20 dark:border-zinc-700/20" : ""}`}
											>
												{m.attachment.type ===
													"image" && (
												...
												{m.attachment.type ===
													...
													<div className="rounded-lg overflow-hidden border border-zinc-200/30 dark:border-zinc-800/30 max-w-full sm:max-w-sm bg-black">
														...
														<div className="p-2 bg-zinc-800 text-white text-[10px] font-bold flex justify-between items-center">
															...
															<span className="text-[8px] bg-red-500 text-white px-1 py-0.5 rounded font-extrabold">
																MP4
															</span>
														</div>
													</div>
														...
														<a
															...
															onClick={(e) => {
																if (
																	...
																	e.preventDefault();
																	toast.success(
																		"Downloading simulated document specs!",
																	);
																}
															}}
															target="_blank"
					...
					<div className="p-4 border-t border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/40 relative">
						...
						>
							...
							>
								<svg
									className="w-4 h-4 transform rotate-90 text-current"
									fill="none"
									viewBox="0 0 24 24"
									stroke="currentColor"
								>
								...
								{activeChat.isGroup ? (
									<div className="space-y-1.5">
										...
										<p className="text-[10px] text-zinc-650 dark:text-zinc-350 leading-relaxed italic">
											This channel connects verified
											wholesale dealers to coordinate B2B
									...
									{activeChat.groupMembers?.map(
										(memberId) => {
											...
											return (
												<div
													key={memberId}
													className="flex items-center gap-2.5 p-2 rounded-xl bg-zinc-50/50 dark:bg-zinc-850/30 border border-zinc-150/50 dark:border-zinc-800/50 hover:bg-zinc-100/30 dark:hover:bg-zinc-800/30 transition-all duration-300"
												>
													{member.profileImg ? (
														<img
															src={
															...
															alt={member.name}
															className="h-7 w-7 rounded-lg object-cover border border-zinc-200/50 dark:border-zinc-800/50"
														/>
													) : (
														<div className="h-7 w-7 rounded-lg bg-gradient-to-tr from-primary/10 to-secondary/10 dark:from-primary/20 dark:to-secondary/20 text-primary dark:text-secondary flex items-center justify-center font-bold text-[10px] shadow-xs">
															{member.initials}
														</div>
													)}
													<div className="min-w-0 flex-1">
														...
														<p className="text-[9px] text-zinc-450 dark:text-zinc-450 truncate leading-tight mt-0.5">
															{member.shopName}
														</p>
													</div>
												</div>
						...
						<div className="flex justify-between items-start mb-4">
							<div>
								...
								<p className="text-[10px] text-zinc-400 mt-1">
									...
									with a verified dealer in the Mobora
									network.
								</p>
							</div>
							<button
								...
								<div>
									...
									{vendors.length > 0 ? (
											...
											{vendors.map((v) => {
												...
												return (
													...
													>
														...
														<div className="min-w-0 flex-1">
															<h5 className="font-bold text-xs text-zinc-900 dark:text-zinc-50 truncate">
																{v.name}
															</h5>
										...
										<div className="text-center py-8 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl text-zinc-400 text-xs italic">
											No other registered trade vendors
											found.
										</div>
									)}
								</div>
