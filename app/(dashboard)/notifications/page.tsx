			...
			<div className="flex-1 min-w-0">
				<div className="flex items-start justify-between gap-3">
					<div className="min-w-0 flex-1">
						<p
							className={`text-sm font-bold truncate ${notif.isRead ? "text-zinc-600 dark:text-zinc-300" : "text-zinc-900 dark:text-zinc-50"}`}
						>
							{notif.title}
						</p>
						...
						<p
							className={`text-xs mt-1 leading-relaxed ${notif.isRead ? "text-zinc-400 dark:text-zinc-500" : "text-zinc-600 dark:text-zinc-300"}`}
						>
							{notif.body}
						</p>
					</div>
				...
				{FILTER_TABS.map((tab) => {
					const count =
						tab.key === "all"
							...
							: notifications.filter((n) => n.type === tab.key)
								  .length;
