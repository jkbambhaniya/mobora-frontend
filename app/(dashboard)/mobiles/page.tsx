...
export default function MobilesPage() {
  ...
  const modelGroups = useMemo(() => {
    const groups: { [key: string]: { brand: string; model: string; items: MobileListing[] } } = {};
  ...
  const filteredModelGroups = useMemo(() => {
    return modelGroups.filter((g) => {
      ...
      const matchesBrand = brandFilter === "All" || g.brand === brandFilter;

      let matchesStatus = true;
      if (statusFilter === "Active") {
        matchesStatus = g.items.some((d) => d.status === "Active" && d.stock > 0);
  ...
  const metrics = useMemo(() => {
    const activeStock = devices.filter((d) => d.status === "Active").reduce((sum, d) => sum + d.stock, 0);
    ...
    return { activeStock, totalSold, totalUniqueModels, totalTracedDevices };
  }, [devices, modelGroups]);
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="p-5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-sm relative overflow-hidden">
          ...
          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block">Unique Catalog Models</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-extrabold tracking-tight">{metrics.totalUniqueModels}</span>
        ...
        <div className="p-5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-sm relative overflow-hidden">
          ...
          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block">Active Inventory</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-extrabold tracking-tight">{metrics.activeStock}</span>
        ...
        <div className="p-5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-sm relative overflow-hidden">
          ...
          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block">Total Units Sold</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-extrabold tracking-tight">{metrics.totalSold}</span>
        ...
        <div className="p-5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-sm relative overflow-hidden">
          ...
          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block">IMEI Logged Devices</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-extrabold tracking-tight">{metrics.totalTracedDevices}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredModelGroups.length > 0 ? (
          filteredModelGroups.map((g) => {
            ...
            const minPrice = Math.min(...g.items.map((d) => d.price));
            const maxPrice = Math.max(...g.items.map((d) => d.price));
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 py-2 border-t border-b border-zinc-100 dark:border-zinc-850/60">
                    <div>
                      <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest block">In Hand</span>
                      <span className="text-sm font-black text-zinc-700 dark:text-zinc-200">{activeCount} units</span>
                    </div>
                    <div>
                      <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest block">Sold Out</span>
                      <span className="text-sm font-black text-zinc-700 dark:text-zinc-200">{soldCount} units</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
                    <span>Pricing:</span>
                    <span className="font-bold text-zinc-900 dark:text-zinc-100" suppressHydrationWarning>
                      {minPrice === maxPrice
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    router.push(`/mobiles/${encodeURIComponent(g.brand)}/${slugify(g.model)}`);
                  }}
          ...
          <div className="col-span-full text-center py-16 bg-white dark:bg-zinc-900/30 rounded-2xl border border-zinc-200/50 dark:border-zinc-800/50 space-y-3">
            <h4 className="font-bold text-zinc-900 dark:text-zinc-200">No Mobile Models Found</h4>
