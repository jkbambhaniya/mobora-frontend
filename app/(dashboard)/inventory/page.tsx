...
import { useDashboard, MobileListing, Customer, slugify } from "@/context/dashboard-context";
...
export default function InventoryPage() {
  ...
  const [sellingDevice, setSellingDevice] = useState<MobileListing | null>(null);
  ...
  const [buybackDevice, setBuybackDevice] = useState<MobileListing | null>(null);
  ...
  const handleSellSubmit = (e: React.FormEvent) => {
    ...
    triggerToast(`Recorded sale of ${sellingDevice.brand} ${sellingDevice.model} to ${sellCustomer}.`);
  };

  const handleOpenBuyback = (device: MobileListing) => {
    setBuybackDevice(device);
    setBuybackPrice(device.purchasePrice ? Math.round(device.purchasePrice * 0.9).toString() : "");
    ...
    if (!originalBuyer) {
      const matchingOrder = orders.find(
        (o) =>
          o.device.toLowerCase().includes(device.model.toLowerCase()) ||
  ...
  const handleBuybackSubmit = (e: React.FormEvent) => {
    ...
    handleAddTradeTransaction({
      ...
      amount: priceNum,
      date: buybackDate,
      notes: buybackNotes || `Re-acquired via buyback from ${buybackCustomer}.`,
    ...
    triggerToast(`Recorded buyback of ${buybackDevice.brand} ${buybackDevice.model} from ${buybackCustomer}.`);
  };
  ...
  const handleDelete = (id: string, name: string) => {
    if (confirm(`Are you sure you want to delete ${name} from catalog?`)) {
      ...
      triggerToast(`Deleted ${name} listing.`);
    }
  };
  ...
  // Metrics
  const activeStockCount = devices.filter((d) => d.status === "Active").reduce((sum, d) => sum + d.stock, 0);
  const soldCount = devices.filter((d) => d.status === "Sold").length;
  const avgPrice = devices.length > 0 ? Math.round(devices.reduce((sum, d) => sum + d.price, 0) / devices.length) : 0;
  const uniqueModelsCount = Array.from(new Set(devices.map((d) => d.brand + " " + d.model))).length;
  ...
  const filteredDevices = devices.filter((d) => {
    ...
    const matchesCondition = conditionFilter === "All" || d.condition === conditionFilter;

    return matchesSearch && matchesStatus && matchesBrand && matchesCondition;
  });
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="p-6 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-sm relative group overflow-hidden">
          ...
          <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-550 uppercase tracking-wider block">Available Stock</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-extrabold tracking-tight">{activeStockCount}</span>
            <span className="text-xs text-zinc-400 dark:text-zinc-500">Units in hand</span>
          </div>
        </div>

        <div className="p-6 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-sm relative group overflow-hidden">
          <div className="absolute top-0 left-0 w-1.5 h-full bg-success" />
          <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-550 uppercase tracking-wider block">Total Sold</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-extrabold tracking-tight">{soldCount}</span>
            <span className="text-xs text-zinc-400 dark:text-zinc-500">Completed sales</span>
          </div>
        </div>

        <div className="p-6 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-sm relative group overflow-hidden">
          <div className="absolute top-0 left-0 w-1.5 h-full bg-secondary" />
          <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block">Avg Selling Price</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-extrabold tracking-tight" suppressHydrationWarning>₹{avgPrice.toLocaleString()}</span>
            <span className="text-xs text-zinc-400 dark:text-zinc-500">Catalog average</span>
          </div>
        </div>

        <div className="p-6 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-sm relative group overflow-hidden">
          <div className="absolute top-0 left-0 w-1.5 h-full bg-purple-500" />
          <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-555 uppercase tracking-wider block">Unique Models</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-extrabold tracking-tight">{uniqueModelsCount}</span>
            <span className="text-xs text-zinc-400 dark:text-zinc-500">Specs cataloged</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 overflow-hidden shadow-sm">
        {filteredDevices.length > 0 ? (
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="bg-zinc-50/50 dark:bg-zinc-900/20 text-zinc-400 font-semibold text-xs uppercase border-b border-zinc-200/40 dark:border-zinc-800/40">
                  ...
                  <th className="py-4 px-6">Inventory Specifications</th>
              ...
              <tbody>
                {filteredDevices.map((d) => (
                  ...
                  >
                    <td className="py-4 px-6 font-mono font-bold text-zinc-400 text-xs">{d.id}</td>
                    <td className="py-4 px-6">
                      <div>
                        ...
                        <div className="text-[11px] text-zinc-400 mt-1">
                          {d.storage} / {d.ram} RAM &bull; {d.color}
                        </div>
                      </div>
                    </td>
          ...
          <div className="text-center py-16 space-y-3">
            <h4 className="font-bold text-zinc-900 dark:text-zinc-200">No Inventory Items Listed</h4>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 max-w-sm mx-auto">
              We couldn&apos;t find any inventory items matching your filters. Try clearing search query.
            </p>
          </div>
        )}
      </div>
      ...
      <Modal
        ...
        title={editingDevice ? `Edit Inventory Item: ${editingDevice.brand} ${editingDevice.model}` : "Register Stock Item"}
        size="lg"
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1 relative">
              <div className="flex justify-between items-center">
                <label className="text-xs font-semibold text-zinc-455 uppercase tracking-wide">Brand *</label>
                <button
                  type="button"
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                <label className="text-xs font-semibold text-zinc-455 uppercase tracking-wide">Model *</label>
                <button
                  type="button"
                  disabled={!formBrand}
          ...
          <form onSubmit={handleSellSubmit} className="space-y-5">
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-800/30 border border-zinc-200/50 dark:border-zinc-800 rounded-2xl flex flex-col gap-2 shadow-inner">
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-zinc-200/40 dark:border-zinc-800/40 pt-2.5 mt-1 text-[11px]">
                <div>
                  <span className="text-zinc-400 block">IMEI:</span>
                  <span className="font-mono text-zinc-700 dark:text-zinc-300 font-bold">{sellingDevice.imei || "N/A"}</span>
                </div>
                <div>
                  <span className="text-zinc-400 block">Cost Price:</span>
                  <span className="font-semibold text-zinc-700 dark:text-zinc-300">₹{sellingDevice.purchasePrice?.toLocaleString() || "-"}</span>
                </div>
                <div>
                  <span className="text-zinc-400 block">Listed Price:</span>
                  <span className="font-extrabold text-primary">₹{sellingDevice.price.toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 text-left">
              ...
              <div className="space-y-1.5 relative">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Customer *</label>
                  <button
                    type="button"
                ...
                {isAddingCust ? (
                  <div className="p-4 border border-primary/20 bg-primary/5 rounded-2xl space-y-3 animate-scaleUp">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-[10px] font-semibold text-zinc-400 uppercase">Full Name</label>
                        <input
                          type="text"
                          value={newCustName}
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-semibold text-zinc-400 uppercase">Phone Number</label>
                        <input
                          type="text"
                          value={newCustPhone}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Actual Selling Price (₹) *</label>
                  <input
                    type="number"
                    required
                    value={sellPrice}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Sale Date *</label>
                  <input
                    type="date"
                    required
                    value={sellDate}
              ...
              {/* Notes */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Transaction Notes</label>
                <textarea
                  value={sellNotes}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-zinc-150 dark:border-zinc-850">
              <Button type="button" variant="ghost" size="sm" onClick={() => setIsSellFormOpen(false)}>
                Cancel
              </Button>
              <Button type="submit" variant="gradient" size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold">
                Confirm Sale
              </Button>
            </div>
          </form>
          ...
          <form onSubmit={handleBuybackSubmit} className="space-y-5">
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-800/30 border border-zinc-200/50 dark:border-zinc-800 rounded-2xl flex flex-col gap-2 shadow-inner">
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-zinc-200/40 dark:border-zinc-800/40 pt-2.5 mt-1 text-[11px]">
                <div>
                  <span className="text-zinc-400 block">IMEI:</span>
                  <span className="font-mono text-zinc-700 dark:text-zinc-300 font-bold">{buybackDevice.imei || "N/A"}</span>
                </div>
                <div>
                  <span className="text-zinc-400 block">Last Cost Price:</span>
                  <span className="font-semibold text-zinc-700 dark:text-zinc-300">₹{buybackDevice.purchasePrice?.toLocaleString() || "-"}</span>
                </div>
                <div>
                  <span className="text-zinc-400 block">Last Sell Price:</span>
                  <span className="font-semibold text-zinc-700 dark:text-zinc-300">₹{buybackDevice.price.toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 text-left">
              ...
              <div className="space-y-1.5 relative">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Customer (Selling back to store) *</label>
                  <button
                    type="button"
                    onClick={() => setIsAddingCust(!isAddingCust)}
                    className="text-[10px] text-primary hover:underline font-bold"
                ...
                {isAddingCust ? (
                  ...
                  <select
                    ...
                    onChange={(e) => setBuybackCustomer(e.target.value)}
                    required
                    className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-850 text-sm focus:ring-2 focus:ring-primary focus:outline-none"
                  >
                    <option value="">-- Choose Client --</option>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Buyback Amount Paid (₹) *</label>
                  <input
                    type="number"
                    required
                    value={buybackPrice}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Buyback Date *</label>
                  <input
                    type="date"
                    required
                    value={buybackDate}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Buyback Condition *</label>
                  <select
                    value={buybackCondition}
                    onChange={(e) => setBuybackCondition(e.target.value as any)}
                    className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-850 text-sm focus:ring-2 focus:ring-primary focus:outline-none"
                  >
                    <option value="Mint">Mint</option>
                    <option value="Excellent">Excellent</option>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Current Battery Health (%) *</label>
                  <input
                    type="number"
                    min={50}
                    max={100}
                    ...
                    onChange={(e) => setBuybackBatteryHealth(parseInt(e.target.value) || 0)}
                    className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-transparent text-sm focus:ring-2 focus:ring-primary focus:outline-none"
                  />
                </div>
              </div>
              ...
              {/* Notes */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">Buyback Notes</label>
                <textarea
                  value={buybackNotes}
            ...
            {/* Actions */}
            <div className="flex justify-end gap-3 pt-4 border-t border-zinc-150 dark:border-zinc-850">
              <Button type="button" variant="ghost" size="sm" onClick={() => setIsBuybackFormOpen(false)}>
                Cancel
              </Button>
              <Button type="submit" variant="gradient" size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold">
                Confirm Buyback
              </Button>
            </div>
          </form>
