...
function DashboardInnerLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`flex-1 flex flex-col lg:flex-row relative bg-zinc-50/50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 transition-colors duration-300 ${isChat ? "h-screen overflow-hidden" : "min-h-screen"}`}>
      
      {/* Fixed Desktop / Fluid Mobile Sidebar */}
      <Sidebar />

      {/* Workspace Content Panel wrapper to offset fixed sidebar */}
      <div className={`flex-1 lg:pl-72 flex flex-col ${isChat ? "h-screen overflow-hidden" : "min-h-screen"}`}>
        <main className={`flex-1 max-w-[1700px] w-full mx-auto flex flex-col ${
          isChat 
