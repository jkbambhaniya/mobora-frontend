...
export default function ProfilePage() {
  ...
  if (isLoadingVendor) {
    return (
      <div className="flex flex-col items-center justify-center p-12 min-h-[400px]">
        ...
        <span className="text-xs text-zinc-400 mt-3 font-semibold animate-pulse">Loading Profile...</span>
      </div>
    );
  }
  ...
  const handleAccountSubmit = async (data: { name: string; email: string }) => {
    ...
    try {
      ...
      if (response.success && response.data?.success) {
        ...
        triggerToast(response.errorData?.message || "Failed to update account details.", "error");
      }
    } catch (err: any) {
  ...
  const handleBusinessSubmit = async (data: {
    ...
    try {
      ...
      if (response.success && response.data?.success) {
        ...
        triggerToast(response.errorData?.message || "Failed to update business details.", "error");
      }
    } catch (err: any) {
      triggerToast("An unexpected error occurred.", "error");
    } finally {
  ...
  const handleImageChange = async (base64String: string | null) => {
    ...
    try {
      ...
      if (response.success && response.data?.success) {
        setVendor(response.data.vendor);
        triggerToast(base64String ? "Profile image uploaded successfully." : "Profile image removed.");
      } else {
        triggerToast(response.errorData?.message || "Failed to update profile image.", "error");
      }
    } catch (err: any) {
      triggerToast("An unexpected error occurred.", "error");
    } finally {
  ...
  const handlePasswordSubmit = async (data: any) => {
    ...
    try {
      ...
      if (response.success && response.data?.success) {
        ...
        triggerToast(response.errorData?.message || "Failed to change password. Make sure current password is correct.", "error");
      }
    } catch (err: any) {
      triggerToast("An unexpected error occurred.", "error");
    } finally {
        ...
        <div className="lg:col-span-4 bg-white dark:bg-zinc-900/30 backdrop-blur-md rounded-3xl overflow-hidden shadow-2xl shadow-zinc-200/80 dark:shadow-none border-0">
          ...
          <div className="p-6 pt-0 relative flex flex-col items-center text-center">
            ...
            <nav className="w-full mt-8 flex flex-col gap-1.5">
              {(["account", "business", "security"] as TabType[]).map((tab) => {
                const isActive = activeTab === tab;
                const getLabel = () => {
                  if (tab === "account") return "Account Details";
                  if (tab === "business") return "Business Profile";
                ...
                return (
                  ...
                  >
                    <span className={`${isActive ? "text-primary dark:text-secondary" : "text-zinc-400"}`}>
                      {getIcon()}
                    </span>
                    <span>{getLabel()}</span>
                  </button>
          ...
          <div className="animate-fadeIn">
            {activeTab === "account" && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">Personal Account Details</h3>
                  <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-0.5">Manage your user registration credentials.</p>
                </div>
                <AccountDetails
                  initialData={{ name: vendor.name, email: vendor.email }}
            ...
            {activeTab === "business" && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">Business / Store Profile</h3>
                  <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-0.5">Configure public contact details and customer-facing billing settings.</p>
                </div>
                <BusinessDetails
                  initialData={{
            ...
            {activeTab === "security" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-0.5">Ensure your portal account is secure by rotating your password.</p>
                </div>
                <SecuritySettings
