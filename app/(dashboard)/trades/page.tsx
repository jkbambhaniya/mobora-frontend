...
import { useDashboard, TradeTransaction, Customer } from "@/context/dashboard-context";
...
export default function TradesTrackerPage() {
  ...
  useEffect(() => {
    if (formImei.length === 15) {
      ...
      if (matchingTrades.length > 0) {
        // Sort newest first
        const latestTrade = matchingTrades.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
        ...
        setAutofillMessage("🆕 New IMEI detected. Please specify specifications manually.");
      }
    } else {
      setAutofillDetected(false);
      setAutofillMessage("");
  ...
  const handleRecordTransaction = (e: React.FormEvent) => {
    ...
    if (!formBrand || !formModel || !formCustomerName || !formAmount) {
      alert("Please enter brand, model, customer name, and transaction amount.");
      return;
    }
  ...
  const computedTimeline = timelineEvents.map((event, idx) => {
    ...
    if (event.type === "Sale") {
      ...
      const prePurchases = timelineEvents.slice(0, idx).filter((t) => t.type === "Purchase");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-zinc-900/50 p-6 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 space-y-6 shadow-sm">
            <div>
              <h2 className="text-lg font-bold text-zinc-900 dark:text-white">Record Buyback & Sales Transactions</h2>
              <p className="text-xs text-zinc-400 mt-1">
                Enter an IMEI to track. System will auto-fill previously logged device specs.
              </p>
            </div>
            ...
            <form onSubmit={handleRecordTransaction} className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-zinc-450 uppercase tracking-wide">15-Digit IMEI *</label>
                <input
                  type="text"
                  maxLength={15}
                  value={formImei}
                ...
                {autofillMessage && (
                  <div className={`mt-2 p-3 rounded-xl border text-xs font-semibold animate-scaleUp ${
                    autofillDetected 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-zinc-400 uppercase">Current Condition</label>
                  <select
                    value={formCondition}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1 relative">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-semibold text-zinc-400 uppercase">Customer *</label>
                    <button
                      type="button"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-zinc-400 uppercase">
                    {txType === "Purchase" ? "Purchase Price (₹) *" : "Sale price (₹) *"}
                  </label>
                  <input
                    type="number"
                    required
                    ...
                    placeholder={txType === "Purchase" ? "e.g. 40000" : "e.g. 50000"}
                    className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-transparent text-sm focus:ring-2 focus:ring-primary focus:outline-none"
                  />
                </div>
                ...
                {/* Date */}
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-zinc-400 uppercase">Tx Date *</label>
                  <input
                    type="date"
                    required
                    value={formDate}
              ...
              {/* Notes */}
              <div className="space-y-1">
                <label className="text-xs font-semibold text-zinc-400 uppercase">Transaction Notes</label>
                <textarea
                  value={formNotes}
              ...
              <Button type="submit" variant="gradient" className="w-full py-3 rounded-xl font-bold cursor-pointer">
                {txType === "Purchase" ? "Record Purchase Buyback" : "Record Outgoing Sale"}
              </Button>
            </form>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900/50 p-6 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 shadow-sm space-y-6">
            <div>
              <h2 className="text-lg font-bold text-zinc-900 dark:text-white">IMEI Lifecycle Trace</h2>
              <p className="text-xs text-zinc-400 mt-1">
                Enter an IMEI to view the complete history of its ownership changes and cumulative profit.
              </p>
            </div>
              ...
              <div className="space-y-5 animate-scaleUp">
                {timelineEvents.length > 0 ? (
                  <div className="space-y-6">
                    ...
                    <div className="p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-250/20 space-y-1.5">
                      ...
                      <div className="flex items-baseline justify-between mt-2 pt-2 border-t border-zinc-200/40">
                        <span className="text-xs text-zinc-400">Total Cycles:</span>
                        <span className="font-bold text-xs">
                          {Math.ceil(timelineEvents.length / 2)}
                        </span>
                      </div>
                      <div className="flex items-baseline justify-between">
                        <span className="text-xs text-zinc-400 font-semibold">Cumulative Profit:</span>
                        <span className={`font-extrabold text-sm ${totalProfit >= 0 ? "text-emerald-500" : "text-red-500"}`}>
                        ...
                        return (
                          <div key={ev.id} className="relative">
                            ...
                            <span className={`absolute -left-[31px] top-1.5 flex h-4 w-4 items-center justify-center rounded-full border bg-white dark:bg-zinc-900 ${
                              ...
                              <span className={`h-1.5 w-1.5 rounded-full ${isPurchase ? "bg-cyan-500" : "bg-emerald-500"}`} />
                            </span>
                            ...
                            <div className="space-y-1.5">
                              <div className="flex items-center justify-between text-[10px] font-bold text-zinc-400">
                                ...
                                <span className={`px-2 py-0.5 rounded-full ${
                                  ...
                                  {isPurchase ? "BUYBACK" : "SOLD"}
                                </span>
                              </div>
                              <div className="text-xs text-zinc-800 dark:text-zinc-200 leading-normal">
                                {isPurchase ? (
                                  <>
                                    Purchased from <strong className="font-bold">{ev.customerName}</strong> for <strong className="font-bold">₹{ev.amount.toLocaleString()}</strong>.
                                  </>
                                ) : (
                                  <>
                                    Sold to <strong className="font-bold">{ev.customerName}</strong> for <strong className="font-bold">₹{ev.amount.toLocaleString()}</strong>.
                                  </>
                                )}
                              </div>
                              ...
                              <div className="flex flex-wrap gap-2 text-[10px] text-zinc-400">
                                {ev.condition && <span>Cond: {ev.condition}</span>}
                                {ev.batteryHealth && <span>Battery: {ev.batteryHealth}%</span>}
                                {ev.margin > 0 && (
                                  <span className="text-emerald-500 font-bold">
                                    Margin: +₹{ev.margin.toLocaleString()}
                                  </span>
                                )}
                              </div>
                              {ev.notes && (
                  ...
                  <div className="text-center py-8 text-zinc-400 space-y-2 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl">
                    <p className="text-xs">No transaction history found for IMEI &quot;{tracedImei}&quot;</p>
                  </div>
                )}
              </div>
            )}
