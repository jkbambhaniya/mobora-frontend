...
import { AdminChatProvider, useAdminChats } from "@/context/admin/chat-context";
			...
			<aside className={`w-64 h-full bg-white dark:bg-[#13151a]/50 backdrop-blur-xl flex flex-col shrink-0 shadow-xl shadow-zinc-200/30 dark:shadow-black/20 z-40 fixed inset-y-0 left-0 transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static transition-transform duration-300 ease-in-out`}>
				<div className="h-16 px-6 flex items-center justify-between gap-3 border-b border-zinc-100 dark:border-white/5 lg:border-none">
					...
					<a
						href="/admin/dealer"
						className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all ${
							pathname.includes("/admin/dealer")
								? "bg-indigo-50 dark:bg-indigo-600/10 border border-indigo-100 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400"
								: "text-zinc-600 dark:text-gray-400 hover:bg-zinc-50 dark:hover:bg-white/5 hover:text-zinc-950 dark:hover:text-white"
						}`}
					>
					...
					<a
						href="/admin/courier"
						className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all ${
							pathname.includes("/admin/courier")
								? "bg-amber-50 dark:bg-amber-600/10 border border-amber-100 dark:border-amber-500/20 text-amber-600 dark:text-amber-400"
								: "text-zinc-600 dark:text-gray-400 hover:bg-zinc-50 dark:hover:bg-white/5 hover:text-zinc-950 dark:hover:text-white"
						}`}
					>
						<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
					...
					>
						<div className="relative">
							...
							{hasUnreadMessages && (
								<span className="absolute -top-0.5 -right-0.5 flex h-2.5 w-2.5 rounded-full bg-red-500 animate-pulse border-2 border-white dark:border-[#13151a]" />
							)}
						</div>
						<span>Chat</span>
					</a>
