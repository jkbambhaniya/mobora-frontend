    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p className={`text-[11px] leading-tight font-bold truncate ${notif.isRead ? "text-zinc-600 dark:text-zinc-400" : "text-zinc-900 dark:text-zinc-50"}`}>
            {notif.title}
          </p>
          <button
            onClick={(e) => {
        ...
        <p className={`text-[10px] mt-0.5 leading-relaxed line-clamp-2 ${notif.isRead ? "text-zinc-400 dark:text-zinc-500" : "text-zinc-600 dark:text-zinc-300"}`}>
          {notif.body}
        </p>
        <span className="text-[9px] text-zinc-400 mt-1 block font-semibold">{notif.timestamp}</span>
      </div>
      {!notif.isRead && (
...
export function NotificationBell() {
  const { notifications, unreadCount, markAllRead, markRead, clearNotification, clearAll } = useNotifications();
    ...
    <div className="relative">
      ...
      {isOpen && (
        <div
          ...
          style={{
            animation: "notifSlideDown 0.18s cubic-bezier(0.16,1,0.3,1) forwards",
          }}
        >
          {/* Panel Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-900/80 backdrop-blur-sm">
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4 text-primary dark:text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
              </svg>
              <h3 className="text-xs font-extrabold text-zinc-900 dark:text-white">Notifications</h3>
              {unreadCount > 0 && (
            ...
            {notifications.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 gap-3 text-zinc-400">
                ...
                <div className="text-center">
                  <p className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400">All caught up!</p>
                  <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-0.5">No notifications yet.</p>
                </div>
              </div>
            ) : (
