...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  const deviceCommands: CommandItem[] = devices.map((dev) => {
    const isImeiSearch = search.trim() && dev.imei && dev.imei.includes(search.trim()) && /^\d+$/.test(search.trim());
    return {
      id: `dev-${dev.id}`,
      category: "Devices",
      label: isImeiSearch ? `View Lifecycle History: ${dev.brand} ${dev.model}` : `${dev.brand} ${dev.model}`,
      ...
      action: () => {
        if (isImeiSearch && dev.imei) {
          router.push(`/mobiles/${encodeURIComponent(dev.brand)}/${slugify(dev.model)}/${dev.imei}`);
        } else {
          router.push(`/mobiles/${encodeURIComponent(dev.brand)}/${slugify(dev.model)}`);
        }
        onClose();
      },
    };
  ...
  // Combine items
  const allItems = [...navigationCommands, ...quickActions, ...deviceCommands];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-zinc-950/60 backdrop-blur-md animate-fadeIn">
      ...
      >
        ...
        <div ref={listRef} className="flex-1 overflow-y-auto p-2.5 max-h-[50vh] space-y-3">
          {filteredItems.length === 0 ? (
            <div className="text-center py-8 text-zinc-400 dark:text-zinc-500">
              <svg className="w-8 h-8 mx-auto stroke-zinc-400 mb-2 opacity-50" fill="none" viewBox="0 0 24 24" strokeWidth="1.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
              </svg>
              <p className="text-xs font-semibold">No results matching &quot;{search}&quot;</p>
              <p className="text-[10px] text-zinc-500 mt-1">Try typing another keyboard key or search parameter.</p>
            </div>
          ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          {/* Item Icon */}
                          <div className={`p-1.5 rounded-lg shrink-0 ${isSelected ? "text-primary dark:text-secondary bg-white dark:bg-zinc-850 shadow-sm" : "text-zinc-450 dark:text-zinc-500 bg-zinc-50 dark:bg-zinc-900"}`}>
                            {item.icon}
                          </div>
                          ...
                          <div className="min-w-0">
                            ...
                            {item.subLabel && (
                              <p className={`text-[10px] mt-0.5 truncate ${isSelected ? "text-primary/75 dark:text-secondary/75" : "text-zinc-400 dark:text-zinc-500"}`}>
                                {item.subLabel}
                              </p>
                            )}
                          </div>
                        ...
                        {/* Shortcut badge hint */}
                        {item.badge && (
                          <span className={`text-[8px] font-mono px-2 py-0.5 rounded border tracking-wider shrink-0 transition-colors ${
                            isSelected
        ...
        <div className="px-4 py-2 bg-zinc-50 dark:bg-zinc-950/50 border-t border-zinc-150 dark:border-zinc-850 flex items-center justify-between text-[9px] text-zinc-450 dark:text-zinc-500 font-semibold tracking-wide">
          <div className="flex items-center gap-3.5">
            <span className="flex items-center gap-1"><kbd className="bg-zinc-100 dark:bg-zinc-900 px-1 py-0.5 rounded border border-zinc-250/20 shadow-sm">↑↓</kbd> Navigate</span>
            <span className="flex items-center gap-1"><kbd className="bg-zinc-100 dark:bg-zinc-900 px-1 py-0.5 rounded border border-zinc-250/20 shadow-sm">↵</kbd> Select</span>
            <span className="flex items-center gap-1"><kbd className="bg-zinc-100 dark:bg-zinc-900 px-1 py-0.5 rounded border border-zinc-250/20 shadow-sm">Esc</kbd> Close</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span>Global Shortcuts Enabled:</span>
            <kbd className="bg-zinc-200 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 px-1 rounded shadow-sm">G</kbd>
            <span>+</span>
            <kbd className="bg-zinc-200 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 px-1 rounded shadow-sm">Key</kbd>
          </div>
        </div>
      </div>
    </div>
