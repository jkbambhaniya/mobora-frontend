...
export const Modal: React.FC<ModalProps> = ({
  ...
  const modalContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/70 backdrop-blur-md overflow-y-auto animate-fadeIn">
      ...
      >
        {/* Header — shown when a title is provided; close button always rendered */}
        <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 p-6 pb-4 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="ml-auto text-zinc-400 hover:text-zinc-600 dark:hover:text-white cursor-pointer flex-shrink-0"
            aria-label="Close"
          >
  ...
  const drawerContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-zinc-950/70 backdrop-blur-md animate-fadeIn">
      ...
      <div
        ...
        onClick={(e) => e.stopPropagation()}
      >
        {/* Drawer Header — close button always rendered; title optional */}
        <div className="p-6 border-b border-zinc-100 dark:border-zinc-850 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-1 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-600 dark:hover:text-white transition-colors cursor-pointer flex-shrink-0"
            aria-label="Close"
          >
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
